### p(theta,x_[1:t]|y_[1:t]); N=N_theta=N_x
smc2<-function(N,T,y,df,sigma_rho,sigma_tau,sigma_sigma){
	start <- Sys.time ()
	...
	low.x=apply(Array[,,1],2,quantile,probs=0.025)
	up.x=apply(Array[,,1],2,quantile,probs=0.975)
	return(list(rh,sig,ta,loglik,count,mean.x,low.x,up.x,p))
}
...
smc2_pred<-function(N,T,y,df,sigma_rho,sigma_tau,sigma_sigma,h){
	...
	if(h==1){
		...
		low.y.pred=apply(Array[,,5],2,quantile,prob=0.15)
		up.y.pred=apply(Array[,,5],2,quantile,prob=0.85)
	}else{
		mean.y.pred=colMeans(Array_temp[,,2])[1:(T+1)]
		low.y.pred=apply(Array_temp[,,2],2,quantile,prob=0.15)[1:(T+1)]
		up.y.pred=apply(Array_temp[,,2],2,quantile,prob=0.85)[1:(T+1)]
	}
	p=log(colMeans(w)) #p(y_[t+1]|y_[0:t])
...
gmhp.one<-function(rho_0,tau_0,sigma_0,sigma_rho,sigma_tau,sigma_sigma,N,y){
	...
	X=rho_0; Z=tau_0; V=sigma_0 #rho, tau, sigma
    		...
    		U1 <- runif(1)
    		if (U1<=min(p(rho=X,tau=W,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(X,min=-1,max=1)*densigamma(W,1,1)*densigamma(V,1,1)*dlnorm(W,meanlog=log(Z)-sigma_tau^2/2,sdlog=sigma_tau)/(p(rho=X,tau=Z,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(X,min=-1,max=1)*densigamma(Z,1,1)*densigamma(V,1,1)*dlnorm(Z,meanlog=log(W)-sigma_tau^2/2,sdlog=sigma_tau)),1)){ #tau
        		Z <- W
    		}else{
        		Z <- Z
    		}
 ...
 #NOT SYMMETRIC: proposal for sigma
    		U3 <- runif(1)
    		if (U3<=min(p(rho=X,tau=Z,N=N,T=length(y)-1,y=y,sigma=R,df=df)*dunif(X,min=-1,max=1)*densigamma(Z,1,1)*densigamma(R,1,1)*dlnorm(R,meanlog=log(V)-sigma_sigma^2/2,sdlog=sigma_sigma)/(p(rho=X,tau=Z,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(X,min=-1,max=1)*densigamma(Z,1,1)*densigamma(V,1,1)*dlnorm(V,meanlog=log(R)-sigma_sigma^2/2,sdlog=sigma_sigma)),1)){ #sigma
        		V <- R
    		}else{
        		V <- V
    		}	
    		...
    		U2<-runif(1)
    		if (U2<=min(p(tau=Z,rho=Y,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(Y,min=-1,max=1)*densigamma(Z,1,1)*densigamma(V,1,1)/(p(tau=Z,rho=X,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(X,min=-1,max=1)*densigamma(Z,1,1)*densigamma(V,1,1)),1)){ #rho
    	 		X <- Y
    		}else{
        		X <- X
    		}

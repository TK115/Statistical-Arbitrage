...
smc2_pred<-function(N,T,y,df,sigma_rho,sigma_tau,sigma_sigma,h,ci.l,ci.u,ess){
	#start <- Sys.time ()
	...
	rho=runif(N,0.7,1); sigma=rigamma(N,10,0.1); tau=rigamma(N,10,0.2)
...
gmhp.one<-function(rho_0,tau_0,sigma_0,sigma_rho,sigma_tau,sigma_sigma,N,y){
 ...
 #NOT SYMMETRIC: proposal for sigma
    		...
    		U2<-runif(1)
    		if (U2<=min(p(tau=Z,rho=Y,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(Y,min=0.7,max=1)*densigamma(Z,10,0.2)*densigamma(V,10,0.1)/(p(tau=Z,rho=X,N=N,T=length(y)-1,y=y,sigma=V,df=df)*dunif(X,min=-0.7,max=1)*densigamma(Z,10,0.2)*densigamma(V,1,1)),1)){ #rho
    	 		X <- Y
    		}else{
        		X <- X
    		}

...
smc2_pred_sv<-function(N,T,y,sigma_rho,sigma_tau,sigma_sigma,h,ci.l,ci.u,ess){
	start <- Sys.time ()
	...
	#rho=runif(N,0.7,0.99); sigma=rigamma(N,10,0.1); tau=rigamma(N,10,0.2)
	rho=runif(N,0.7,0.99); sigma=rigamma(N,5,1); tau=rigamma(N,4,1)
		
	Array[,1,2]=rho; Array[,1,3]=sigma; Array[,1,4]=tau
	Array[,1,1]=rnorm(N,mean=0,sd=sqrt(tau^2/(1-rho^2)))
	Array[,1,5]=rnorm(N,mean=0,sd=sigma*exp(Array[,1,1]/2))
	...
	for(i in 1:T){
		Array[,i+1,1]=rnorm(N,mean=rho*Array[,i,1],sd=tau)
		Array[,i+1,5]=rnorm(N,mean=0,sd=sigma*exp(Array[,i+1,1]/2))
		
		if(h>1){
			Array_temp[,i+1,1]=Array[,i+1,1]
			Array_temp[,i+1,2]=Array[,i+1,5]
			for(j in 1:(h-1)){
				Array_temp[,i+1+j,1]=rnorm(N,mean=rho*Array[,i+j,1],sd=tau)
				Array_temp[,i+1+j,2]=rnorm(N,mean=0,sd=sigma*exp(Array[,i+1+j,1]/2))
			}
		...
		Array[,i+1,3]=sigma
		Array[,i+1,4]=tau
		w[,i+1]=dnorm(yy[,i+1],mean=0,sd=sigma*exp(Array[,i+1,1]/2))
		ww[,i+1]=w[,i+1]/sum(w[,i+1])
		
		if(1/sum(ww[,i+1]^2)<ess*N){
			count=count+1
			...
			Array[,i+1,1]=rnorm(N,mean=Array[,i+1,2]*Array[,i,1],sd=Array[,i+1,4])
			w[,i+1]=dnorm(yy[,i+1],mean=0,sd=Array[,i+1,3]*exp(Array[,i+1,1]/2))
			ww[,i+1]=w[,i+1]/sum(w[,i+1])	
		}
	...
	if(h==1){
		...
		low.y.pred=apply(Array[,,5],2,quantile,prob=ci.l)
		up.y.pred=apply(Array[,,5],2,quantile,prob=ci.u)
	}else{
		mean.y.pred=colMeans(Array_temp[,,2])[1:(T+1)]
		low.y.pred=apply(Array_temp[,,2],2,quantile,prob=ci.l)[1:(T+1)]
		up.y.pred=apply(Array_temp[,,2],2,quantile,prob=ci.u)[1:(T+1)]
	}
...
p<-function(N,T,y,rho,tau,sigma){
	...
	xx[,1]=rnorm(N,mean=0,sd=sqrt(tau^2/(1-rho^2)))
	w[,1]=1/N
	ww[,1]=w[,1]/sum(w[,1])
	xx_res[,1]=xx[,1]
	for(i in 1:T){
		xx[,i+1]=rho*xx_res[,i]+tau*rnorm(N)
		w[,i+1]=dnorm(yy[,i+1],mean=0,sd=sigma*exp(xx[,i+1]/2))
		ww[,i+1]=w[,i+1]/sum(w[,i+1])
		#adaptive resampling
...
# one run of PMCMC transition kernal
gmhp.one<-function(rho_0,tau_0,sigma_0,sigma_rho,sigma_tau,sigma_sigma,N,y){
	...
	X=rho_0; Z=tau_0; V=sigma_0 #rho, tau, sigma
    		...
    		U1 <- runif(1)
    		if (U1<=min(p(rho=X,tau=W,N=N,T=length(y)-1,y=y,sigma=V)*dunif(X,min=0.7,max=1)*densigamma(W,10,0.2)*densigamma(V,10,0.1)*dlnorm(W,meanlog=log(Z)-sigma_tau^2/2,sdlog=sigma_tau)/(p(rho=X,tau=Z,N=N,T=length(y)-1,y=y,sigma=V)*dunif(X,min=0.7,max=1)*densigamma(Z,10,0.2)*densigamma(V,10,0.1)*dlnorm(Z,meanlog=log(W)-sigma_tau^2/2,sdlog=sigma_tau)),1)){ #tau
        		Z <- W
    		}else{
        		Z <- Z
    		}
 ...
 #NOT SYMMETRIC: proposal for sigma
    		U3 <- runif(1)
    		if (U3<=min(p(rho=X,tau=Z,N=N,T=length(y)-1,y=y,sigma=R)*dunif(X,min=0.7,max=1)*densigamma(Z,10,0.2)*densigamma(R,10,0.1)*dlnorm(R,meanlog=log(V)-sigma_sigma^2/2,sdlog=sigma_sigma)/(p(rho=X,tau=Z,N=N,T=length(y)-1,y=y,sigma=V)*dunif(X,min=0.7,max=1)*densigamma(Z,10,0.2)*densigamma(V,10,0.1)*dlnorm(V,meanlog=log(R)-sigma_sigma^2/2,sdlog=sigma_sigma)),1)){ #sigma
        		V <- R
    		}else{
        		V <- V
    		}	
    		...
    		U2<-runif(1)
    		if (U2<=min(p(tau=Z,rho=Y,N=N,T=length(y)-1,y=y,sigma=V)*dunif(Y,min=0.7,max=1)*densigamma(Z,10,0.2)*densigamma(V,10,0.1)/(p(tau=Z,rho=X,N=N,T=length(y)-1,y=y,sigma=V)*dunif(X,min=-0.7,max=1)*densigamma(Z,10,0.2)*densigamma(V,1,1)),1)){ #rho
    	 		X <- Y
    		}else{
        		X <- X
    		}
